...
public class USHMMPersonsFeatureExtractor {
    ...
    public Map<ClassificationInstance, Pair<List[], List[]>> getClassificationInstanceUSHMMPersonPairMap(
        ...
        Map<ClassificationInstance, Pair<List[], List[]>> classificationInstanceUSHMMPersonPairMap = new HashMap<>();
        USHMMPersonIndex index = new USHMMPersonIndex(personIdFSABin, indexFileName);

        for (USHMMGoldStandardEntry entry : data) {
            Pair<List[], List[]> personPair = new Pair<>(index.getPerson(entry.personId1), index.getPerson(entry.personId2));
