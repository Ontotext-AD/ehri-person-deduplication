...
class ClusteringResultsWriter {
    ...
    void printResults(String resultsDirectory, List<Cluster> clusters, DBSCANClustering dbscan, int size) {
        try {
            PrintWriter writer = new PrintWriter(getFileNameResults(resultsDirectory), "UTF-8");
            printResultsToFile(writer, clusters, dbscan, size);
            writer.close();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void printResultsToFile(PrintWriter writer, List<Cluster> clusters, DBSCANClustering dbscan, int size) {
        printHeaderInfo(writer, clusters, size, dbscan.eps, dbscan.minPts, dbscan.levenshteinDistance);
    ...
    private void printHeaderInfo(PrintWriter writer, List<Cluster> clusters, int totalPoints, double epsilon, int minimalPoints, double levenshteinDistance) {
        ...
        writer.println("Average points in cluster : " + String.format("%.10f", (double) totalPoints / (clusters.size() + (totalPoints - reachablePointsCount))));
